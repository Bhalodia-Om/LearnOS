...
grub-mkrescue -o build/LeveretOS-StringHelpers.iso build/isodir

echo "[5/5] Launching QEMU..."

# Boot the ISO in our emulated 32-bit PC(represented by i386, representing a 32-bit x86 computer).
qemu-system-i386 -cdrom build/LeveretOS-StringHelpers.iso      # "-cdrom". Attach our ISO as a read only(ROM) disk.